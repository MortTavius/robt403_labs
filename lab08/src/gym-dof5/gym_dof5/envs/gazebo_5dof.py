...
class GazeboPlanar5DofEnv(gym.Env):
	...
	def step(self, action):
		...
		if action == 0: #Base motor left
			msg = angles.position[4] - deg2rad(10) # You can change the sample angle
			while True:
			...
			msg = angles.position[4] + deg2rad(10)
			while True:
				self.motor1_pub.publish(Float64(msg))
				s = rospy.wait_for_message('/robot/joint_states', JointState, timeout = 5)
				if (abs(s.position[4] - msg) < 0.01):
			...
			msg = angles.position[1] - deg2rad(10)
			while True:
			...
			msg = angles.position[2] - deg2rad(10)
			while True:
			...
			msg = angles.position[3] - deg2rad(10)
			while True:
			...
			msg = angles.position[0] - deg2rad(10)
			while True:
